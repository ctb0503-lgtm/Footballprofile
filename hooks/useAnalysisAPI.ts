...
interface UseAnalysisAPIReturn {
  ...
  generateTeamNews: (
    ...
    systemPrompt: string,
    apiKey: string,
  ) => Promise<Profile>; // <-- Corrected return type
  ...
  const askFollowUp = useCallback(
    async (
      ...
      try {
        ...
        throw error; // Re-throw for consistent error handling if caller needs it
      } finally {
  ...
  const generateNews = useCallback(
    async (
      ...
      try {
        ...
        return news; // <-- RETURN the news for sequential processing
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : String(error);
