  ...
  return (
    <div className="relative group">
      ...
      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-gray-950 text-white text-xs rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none border border-gray-700">
        <h5 className="font-bold text-sm mb-2 text-center border-b border-gray-700 pb-1">
          Volatility Breakdown
        </h5>
        <div className="grid grid-cols-2 gap-x-2 gap-y-1">
          <span className="text-gray-400">Avg. Scored:</span>{" "}
          <span className="text-white font-mono">{meanScored.toFixed(2)}</span>
          <span className="text-gray-400">Std. Dev. Scored:</span>{" "}
          <span className="text-white font-mono">
            {stdDevScored.toFixed(2)}
          </span>
          <span className="text-gray-400">Scored CV:</span>{" "}
          <span className="text-white font-mono font-bold">
            {(scoredCV * 100).toFixed(0)}%
          </span>
          <span className="text-gray-400 mt-2">Avg. Conceded:</span>{" "}
          <span className="text-white font-mono mt-2">
            {meanConceded.toFixed(2)}
          </span>
          <span className="text-gray-400">Std. Dev. Conceded:</span>{" "}
          <span className="text-white font-mono">
            {stdDevConceded.toFixed(2)}
          </span>
          <span className="text-gray-400">Conceded CV:</span>{" "}
          <span className="text-white font-mono font-bold">
            {(concededCV * 100).toFixed(0)}%
          </span>
        </div>
      </div>
    </div>
